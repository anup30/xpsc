// D. Addition and Sum (segment tree- lazy propagation), Accepted
...
ll query(int id, int b, int e, int i, int j, int carry=0){
	if (i > e || j < b) return 0; 
	if (b >= i && e <= j) return seg[id].first+ (ll)carry*(e-b+1);
	int mid = (b + e) / 2;
...
void update(int id, int b, int e, int i, int j, int x){ // increment by x in range i , j
	...
	seg[id].first = seg[2*id].first+seg[2*id+1].first+ (ll)(e-b+1)*seg[id].second;
}
